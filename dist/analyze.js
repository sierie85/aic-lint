...
import { DEFAULT_SETTINGS, DEFAULT_THRESHOLDS } from "./settings.js";
...
function finding(level, message, fix) {
    return fix ? { level, message, fix } : { level, message };
}
export function checkAiConfigPresence(config) {
    const hasAny = config.claudeMdFiles.length > 0 ||
...
export function checkClaudeMdLength(config, _fileExists = existsSync, thresholds = DEFAULT_THRESHOLDS) {
    return config.claudeMdFiles.flatMap((file) => {
        ...
        if (file.lineCount > thresholds.claudeMdErrorLines)
            return [finding("ERROR", note)];
        if (file.lineCount > thresholds.claudeMdWarnLines)
            return [finding("WARN", note)];
        return [];
    });
}
export function checkDeadRefs(config, fileExists = existsSync) {
    return markdownFiles(config).flatMap((file) => extractFileRefs(file.content)
        .filter((ref) => !fileExists(join(config.root, ref)))
        .map((ref) => finding("ERROR", `Dead path in ${file.relPath}: \`${ref}\``)));
}
export function checkClaudeMdStructure(config) {
...
export function checkSkillQuality(config) {
    return config.skills
        .filter((skill) => {
        if (skill.lineCount <= SKILL_MIN_LINES)
            return false;
...
export function checkSkillOverlap(config) {
    const findings = [];
...
export function checkRedundancy(config) {
    ...
    for (const file of [...config.claudeMdFiles, ...config.skills]) {
        ...
        for (const raw of file.content.split("\n")) {
            ...
            if (norm.length < REDUNDANCY_MIN_LEN)
                continue;
            if (!filesByLine.has(norm))
                filesByLine.set(norm, new Set());
...
export function checkFrontmatter(config) {
    const findings = [];
...
export function checkJsonConfigs(config) {
    return config.jsonConfigs.flatMap((file) => {
        try {
            ...
            return [finding("ERROR", `${file.relPath}: invalid JSON (${e.message})`)];
        }
    });
}
function isGitignored(entry, patterns) {
    if (patterns.has(entry) || patterns.has(`/${entry}`))
        return true;
    const base = entry.split("/").pop();
    if (patterns.has(base) || patterns.has(`/${base}`))
        return true;
    // A directory pattern (e.g. ".claude/" or "/.claude") covers everything under it.
    for (const p of patterns) {
        const dir = p.replace(/^\//, "").replace(/\/$/, "");
        if (dir && entry.startsWith(`${dir}/`))
            return true;
    }
    return false;
}
export function checkGitignore(config, fileExists = existsSync) {
    const patterns = new Set((config.gitignore?.content ?? "").split("\n").map((l) => l.trim()).filter(Boolean));
    return GITIGNORE_SENSITIVE
        .filter((entry) => fileExists(join(config.root, entry)) && !isGitignored(entry, patterns))
        .map((entry) => finding("WARN", `${entry} exists but is not in .gitignore`, {
        relPath: ".gitignore",
        description: `Add ${entry} to .gitignore`,
        apply: (c) => appendLine(c, entry),
    }));
}
export function checkContextBudget(config, _fileExists = existsSync, thresholds = DEFAULT_THRESHOLDS) {
    ...
    if (total <= thresholds.alwaysOnWarnTokens)
        return [];
...
export function checkSecrets(config) {
    return allFiles(config).flatMap((file) => scanSecrets(file.content).map((m) => finding("ERROR", `Possible secret in ${file.relPath}: ${m.kind} (${m.snippet})`)));
}
...
export function analyze(config, fileExists = existsSync, settings = DEFAULT_SETTINGS) {
